...
function addGamesToPage(games) {
  ...
  for (let i = 0; i < games.length; i++) {
    ...
    const image = `<div > <img src = ${element.img} /> </div>`;
    const html = `<div class = "game-card"> <p> ${image} ${
      element.name
    }</p><p>${element.description}</p><p>${"Backers : "}${
      element.backers
    }</p></div>`;
...
let totalContributions = GAMES_JSON.reduce(
...
let unfundedDescription =
  unfundedGames.length > 0
    ? `${unfundedGames.length} games are still in need of funding.`
    : "All games are fully funded!";

// create a new DOM element containing the template string and append it to the description container
let unfundedDescriptionElement = document.createElement("p");
unfundedDescriptionElement.textContent = unfundedDescription;
...
firstGameElement.textContent = `Top pledged game is ${
  firstGame.name
} with a total of ${firstGame.pledged.toLocaleString()} pledged.`;
firstGameContainer.appendChild(firstGameElement);

// do the same for the runner-up item
let secondGameElement = document.createElement("p");
secondGameElement.textContent = `Second top pledged game is ${
  secondGame.name
} with a total of ${secondGame.pledged.toLocaleString()} pledged.`;
